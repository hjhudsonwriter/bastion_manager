...
function ensureDiplomacyPanel(){
  ...
  panel.innerHTML = `
    ...
    <div id="diplomacyList" class="stack"></div>
  ...
  panel.querySelector("#clearDiplomacyBtn").addEventListener("click", () => {
    if(!confirm("Clear Diplomacy & Trade records? (Does not affect treasury history already gained.)")) return;
...
function renderDiplomacy(){
  ...
  meta.textContent = totalIncome > 0
    ? `Active passive income: +${totalIncome} gp per Bastion Turn.`
  ...
  function block(title, arr, empty){
    if(!arr || arr.length===0){
      blocks.push(`<div class="small muted">${escapeHtml(empty)}</div>`);
      return;
    }
    blocks.push(`<div class="small" style="font-weight:700; margin-top:8px">${escapeHtml(title)}</div>`);
    blocks.push(arr.map(x => {
      const extra = x.incomePerTurn ? ` • +${x.incomePerTurn} gp/turn` : "";
      return `<div class="item">
        <div>
          <div class="item__name">${escapeHtml(x.title || "Record")}</div>
          <div class="item__meta">${escapeHtml((x.clan || x.pair || "—") + " • " + x.turnsLeft + " turns remaining" + extra)}</div>
        </div>
      </div>`;
    }).join(""));
  }

  block("Trade Agreements", d.agreements, "No trade agreements yet.");
  block("Delegations", d.delegations, "No delegations hosted yet.");
  block("Summits", d.summits, "No summits convened yet.");
  block("Arbitration", d.arbitrations, "No arbitration authority active.");
  block("Trade Consortiums", d.consortiums, "No consortiums established.");
